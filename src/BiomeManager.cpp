...
/*   Updated: 2017/11/23 17:05:04 by lfourque         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	if (chunk.isLandscapeSetup() == false) {
		...
		for (int x = 0; x < CHUNK_SIZE; ++x) {
			for (int z = 0; z < CHUNK_SIZE; ++z) {
				if (hasCave) {
					type = (caveNoise.GetNoise(chunkPos.x + x, chunkPos.z + z) + 1.f) / 2.f;
					for (int y = 0; y < CHUNK_SIZE; ++y) {
						height = (caveNoise.GetNoise(chunkPos.x + x, chunkPos.z + z, chunkPos.y + y));
						if (height > 0.0f) {
							if (type < 0.33f) 
					...
					type = (caveNoise.GetNoise(chunkPos.x + x, chunkPos.z + z) + 1.f) / 2.f;
					for (int y = 0; y < CHUNK_SIZE; ++y) {
						height = (caveNoise.GetNoise(chunkPos.x + x, chunkPos.z + z, chunkPos.y + y)) + gradient;
						if (height > 0.0f)
						{
							if (type < 0.33f) 
								chunk.getBlock(x, y, z) = BlockTypes::SAND;
					...
					height = getHeightAt(chunkPos.x + x, chunkPos.z + z);
					type = getMoisture(chunkPos.x + x, chunkPos.z + z);
					for (int y = 0; y < CHUNK_SIZE; ++y) {
